...
* 正则化：在代价函数中添加惩罚项$\lambda\sum_{j=1}^{n}\theta_j^2$；当所有参数都很小的时候，假设会更简单，函数更光滑，更不容易发生过拟合 [Fig 07-04]
* 一般来说不惩罚$\theta_0$
...
* 在梯度下降递归偏导数中加上惩罚项 [Fig 07-06]
* 正规方程中同样加上惩罚项 [Fig 07-07]
* 只要正则化参数是大于零的，正规方程需要求逆矩阵的部分即使$m\leq n$样本数小于变量数，也是可逆的 [Fig 07-08]

### 正则化逻辑回归 Regularized Logistic Regression

* 同样在代价函数中加上惩罚项 [Fig 07-09]
* 在梯度下降的递归函数加上惩罚项的偏导数（和线性回归的方程类似）
...
**神经网络中的常见表示【a表示activation，既是前一层的输出值，也是后一层的输入值；即每一层theta的行数为变量数+1，列数为下一层的单元个数】** [Fig 08-05]
