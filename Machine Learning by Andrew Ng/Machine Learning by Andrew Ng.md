...
## Lecture 08 神经网络 Neural Networks
...
* 高偏差High Bias是欠拟合Underfit，高方差High Variance（即不平滑）是过拟合Overfit【可以说偏差反应了拟合曲线对样本的偏离，偏差大就是拟合得不好；方差反应了拟合曲线本身是否平滑】 [Fig 10-06]
...
* F1值F1 Score/F值F Score：权衡查准率和召回率的值，$F = 2\frac{PR}{P+R}$；不用平均值，因为极端情况平均值很高 [Fig 11-08]

### 机器学习的数据

* 以前的研究表明，大样本数据量足够大的情况下，所有的算法效能都有所提高，甚至样本量小时表现差的算法在样本量大的时候效果会超过其他样本量小时表现更好的算法，即算法的效果有时候依赖一些数据细节，这在下面的论点中得到了更具体的论述（即何时大样本有效，应该是特征涵盖信息足够多的时候） [Fig 11-09]
* 有足够多的特征值可以保证低方差，训练误差很小；在此基础上样本量远大于特征数量，则不易出现过拟合，低偏差，二者同时导致测试误差小，算法效果好 [Fig 11-10] & [Fig 11-11]



## Lecture 12 支持向量机 Support Vector Machine



### 优化目标 Optimization Objective

* 对逻辑回归的代价函数进行修改：将函数变得更简单直接（取到0），同时约掉常数项m，正则化参数也变到了训练代价上（后两者是习惯） [Fig 12-01] & [Fig 12-02] & [Fig 12-03] & [Fig 12-04]

### 大间隔分类器 Large Margin Classifier

* 间距 Margin：支持向量机的决策边界到两组数据的最短距离，这是支持向量机具有鲁棒性的原因【鲁棒性即系统在受到不确定性扰动时仍然能够保持其特征的性质】 [Fig 12-05] & [Fig 12-06]

* 向量的内积：投影长度乘原向量长度，无方向 [Fig 12-07]
* 范数$||u||$：即向量的欧式长度
* 最小化正则化项即是在约束条件下最小化参数的范数，而约束条件是在给定参数范数的情况下，样本到参数的投影应该大于等于1或小于等于-1，这就要求参数向量穿过样本数据，决策边界垂直于参数向量在能够最大分离两组样本的地方 [Fig 12-08] & [Fig 12-09]

### 核函数 Kernels

* 解决非线性分类的高阶项问题 [Fig 12-10]
* 核函数用于非线性分类，是相似度函数，选择标记点 landmarks，再用指数进行分类 [Fig 12-11]
* 高斯核函数 Gaussian kernel：$e^{-\frac{||x-l^{(1)}||^2}{2\sigma ^2}}$

* 对于高斯核函数，样本值离标记点越近，核函数的值越接近于1，越远则趋近于0；$\sigma$越大越敏感 [Fig 12-12] & [Fig 12-13]
* 即将离标记点的综合远近作为新的特征值，用于训练参数

* 如何选择标记点：例如将所有的样本点作为标记点 [Fig 12-14] & [Fig 12-15]
* 从$x^{(i)}$映射到$x^{(i)}$与每个$l^{(j)}$的核函数值构成的$f^{(m+1)}$这个$m+1$维矩阵，再用该矩阵作为新的$x$矩阵（相当于不同阶的项，替换了高次项、交叉项等等）计算某个样本的代价函数 [Fig 12-16] & [Fig 12-17]
* 可以将最后一项写为
* 核函数也可以用在其他回归，但是时间开销不划算，效果在支持向量机里最好
* 支持向量机偏差和方差的取舍：C很大时相当于lambda很小，低偏差高方差；C很小时相当于lambda很大，高偏差低方差；高斯核函数的sigma很大时核函数值f变化缓慢，高偏差低方差；sigma很小时核函数变化快，低偏差高方差 [Fig 12-18]

### 使用支持向量机 Using an SVM

* 一定用第三方包，如`liblinear`和`libsvm`；需要自行选择参数$C$和核函数 [Fig 12-19]
* 线性核函数 linear kernel 也就等于没用核函数，常用在特征量多但是样本量少的情况，仅需要拟合一条决策边界即可，用复杂的非线性函数可能过拟合
* 高斯核函数需要自己选择$\sigma$的值；特征量少时使用
* 大小很不一样的特征量最好均一化 [Fig 12-20]
* 默塞尔定理 Mercer's Theorem：核函数选择需要满足该定理，为了支持支持向量机的数值优化算法 [Fig 12-21]
* 其他核函数：多项式核函数 Polynomial kernel（$k=(x^Tl+c)^n$）；字符串核函数（输入是文本字符串或其他字符串时使用） String kernel；卡方核函数 Chi-square kernel；直方相交核函数 Histogram intersection kernel；总之都是度量两个量（样本和标记点）之间的相似性
* 多元分类：用内置函数或一对多方法 [Fig 12-22]
* 逻辑回归和支持向量机的选择：特征量远大于样本量时用逻辑回归或线性核函数的支持向量机；特征量少样本量适中可以用高斯核函数的支持向量机；特征量少样本量大的时候，可以增加特征量用逻辑回归或线性核函数的支持向量机（高斯核函数的支持向量机会非常慢）；神经网络全都可以用，就是可能比较慢 [Fig 12-23]
* 逻辑回归和线性核函数的支持向量机非常相似，在具体情况下某个的表现可能会更优
* 支持向量机是凸优化，不需要担心局部最优问题（神经网络也不存在局部最优问题）
...
* 例如，想分为两个簇clusters，先随机选择两个点（聚类中心cluster centroids），K均值迭代做簇分配（离哪个中心更近）和移动聚类中心（移动到颜色相同点的均值处），直到点的颜色不改变 [Fig 13-01] & [Fig 13-02] & [Fig 13-03]
* 【K-means太屌了】
* 若存在没有点分配的聚类中心，直接移除；如果实在需要就再随机选择一个
* 对于没有很好分开的簇也可以很好分类 [Fig 13-04]

### 优化目标 Optimization Objective

* 每个点到对应中心距离的平方和 [Fig 13-05]
* 每一步的分类聚类中心也就是在求最小化代价函数的分配方案（给定聚类中心位置）；移动聚类中心也就是在求最小化代价函数的聚类中心位置（给定对应好簇的样本点分布） [Fig 13-06]

### 随机初始化 Random Initialization

* 最好随机选择样本点初始化 [Fig 13-07]
* 避免局部最优：尝试多次初始化，选择代价函数值最小的一个；如果聚类数比较少如2-10，多次随机初始化能保证有比较好的解，如果K非常大可能第一次的效果就很好次数多了也差不多 [Fig 13-08] & [Fig 13-09]

### 聚类数量 Number of Clusters

* 最好通过看可视化图或输出结果判断，即手动选择；目前还没有自动选择合适聚类数量的方法
* 肘部法则 Elbow Method：改变聚类数量K，观察代价函数值J的变化，选择从陡然下降到缓慢下降的转折值【可以认为缓慢下降是因为K增大造成的，而不是样本内涵差别】；也有可能连续，则不好判断 [Fig 13-10]
* 可以根据后续下游目的判断，即具体分类结果是否好用 [Fig 13-11]



## Lecture 14 降维 Dimensionality Reduction



### 数据压缩 Data Compression

* 如果两个量高度相关（如线性关系），可以压缩为一个量 [Fig 14-01]
* 三维的量如果在同一平面或近似同一平面，可以压缩（投影+坐标变换）为二维 [Fig 14-02]
* 多维压缩同理

### 数据可视化 Data Visualization

* 降维归纳然后绘图 [Fig 14-03] & [Fig 14-04] & [Fig 14-05]

### 主成分分析 Principal Component Analysis

* 寻找一个低维面，使得现有数据投影到该维度距离的平方和最小；该距离被称为投影误差 Projection Error，即使投影误差平方和最小 [Fig 14-06]
* 即将n维数据投影到k个向量张开的线性子空间上，使得投影误差最小 [Fig 14-07]
* 主成分分析和线性回归的关系：线性回归的目的是预测，用的是垂直于x轴的距离；主成分分析是降维，用的是到分界空间的垂直距离 [Fig 14-08]

**主成分分析的具体步骤** [Fig 14-09] & [Fig 14-10] & [Fig 14-11] & [Fig 14-12] & [Fig 14-13] & [Fig 14-14]

1. 均值标准化（特征放缩）
2. 计算降维后的空间：首先计算两两数据的协方差矩阵，通过奇异值分解`[U, S, V] = svd()`计算，K维空间即是矩阵U的前K个向量（前K列）
3. 进行坐标变换得到低维坐标$z = U_{reduce}^T x$

### 主成分数量选择

* K即是保留的主成分的数量
* 定义投影误差平方的平均值和数据总方差，选择使二者比值小于0.01即1%的最小的K；这种情况下99%的方差会被保留；也可以取0.05，0.1，0.15等 [Fig 14-15]
* 一般选用0.01或0.05，这时K已经可以取到比较小了，因为真实世界中的数据大多是高度相关的；实际操作中直接从K=1开始向大找第一个满足的即可；可以通过svd得到的矩阵S更方便地计算比值$1-\frac{\sum_1^k S_{ii}}{\sum _1^n S_{ii}}$ [Fig 14-16] & [Fig 14-17]

### 压缩重现

* 给定降维坐标变换$z = U^T_{reduce}x$，还原坐标有$x_{approx}=U_{reduce}z$

### 主成分分析应用建议

* PCA用在训练集上，然后用同样的映射去计算交叉验证集和测试集 [Fig 14-18]
* 应用：压缩内存占用和硬盘占用（处理图像数据）；加速学习算法；可视化 [Fig 14-19]
* 不应该用于防止过拟合（会舍弃部分信息），正则化效果已经很好且不会丢失信息 [Fig 14-20]
* 在用PCA之前想想能不能直接用原始数据 [Fig 14-21]
